...
def get_player_picture(player_id):
    pic = urllib.urlretrieve("http://stats.nba.com/media/players/230x185/" + player_id + ".png", player_id + ".png")
    player_pic = plt.imread(pic[0])
    plt.imshow(player_pic)
    return player_pic


def make_plotly_shot_chart(player_id, year, season_type):
    court_shapes = make_court_shapes()
    shots_df = dg.get_shot_data(player_id=player_id, season_year=year, season_type=season_type)
    player_name = shots_df['PLAYER_NAME'].unique()[0]
    ...
    layout = go.Layout(
        title='Shots by ' + player_name + ' in NBA session 2015-16',
        showlegend=True,
        xaxis=dict(
            showgrid=False,
            range=[-300, 300]
    ...
    url = py.plot(fig, filename='Anthony Davis Shot Chart')
...
def make_matplot_scatter_shot_chart(player_id, year, season_type):
    shot_df = dg.get_shot_data(player_id=player_id, season_year=year, season_type=season_type)
    shot_df = shot_df[shot_df.SHOT_DISTANCE < 40]
    player_name = shot_df['PLAYER_NAME'].unique()[0]

    player_pic = get_player_picture(player_id)
    ...
    file_path = '../shot_charts/' + player_name + '_' + str(year) + '_' + season_type + '_' + 'scatter.png'
    dg.create_directories_and_check_for_file(file_path)
    plt.savefig(file_path)


def make_matplot_hexbin_shot_chart(player_id, year, season_type):
    shot_df = dg.get_shot_data(player_id=player_id, season_year=year, season_type=season_type)
    shot_df = shot_df[shot_df.SHOT_DISTANCE < 40]
    player_name = shot_df['PLAYER_NAME'].unique()[0]
    player_pic = get_player_picture(player_id)
    ...
    joint_shot_chart = sns.jointplot(shot_df.LOC_X, shot_df.LOC_Y, stat_func=None,
                                     kind='hex', space=0, color=cmap(.9), cmap=cmap)
    ...
    file_path = '../shot_charts/' + player_name + '_' + str(year) + '_' + season_type + '_' + 'hex.png'
    dg.create_directories_and_check_for_file(file_path)
    plt.savefig(file_path)


def make_matplot_kde_shot_chart(player_id, year, season_type):
    shot_df = dg.get_shot_data(player_id=player_id, season_year=year, season_type=season_type)
    shot_df = shot_df[shot_df.SHOT_DISTANCE < 40]
    player_name = shot_df['PLAYER_NAME'].unique()[0]
    ...
    file_path = '../shot_charts/' + player_name + '_' + str(year) + '_' + season_type + '_' + 'kde.png'
    dg.create_directories_and_check_for_file(file_path)
    plt.savefig(file_path)


df = dg.get_general_stats(dg.PlayerOrTeam.P, dg.MeasureTypes.BASE, dg.PerModes.TOTAL, 2015, dg.SeasonTypes.REG)
df = df[df.TEAM_ABBREVIATION == 'NOP']
df = df[df.FGA > 300]
player_ids = df['PLAYER_ID'].unique()
for index, pid in enumerate(player_ids):
    make_matplot_scatter_shot_chart(str(pid), 2015, dg.SeasonTypes.REG)
    make_matplot_hexbin_shot_chart(str(pid), 2015, dg.SeasonTypes.REG)
    make_matplot_kde_shot_chart(str(pid), 2015, dg.SeasonTypes.REG)
