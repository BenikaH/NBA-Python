...
def calculate_assist_plus_plus_for_year(year='2016-17'):
    ...
    for ix, player_id in enumerate(player_ids):
        if player_id != 0:
            ...
            for jx, shot in player_df.iterrows():
                shot_df = shots_df[shots_df.PLAYER1_ID == shot.PLAYER1_ID]
                shot_df = shot_df[shot_df.SHOT_ZONE_BASIC == shot.SHOT_ZONE_BASIC]
                shot_percentage = float(len(shot_df[shot_df.SHOT_MADE_FLAG == 1])) / len(shot_df)
                point_value = 3 if '3' in shot.SHOT_ZONE_BASIC else 2
                player_assist_details['ast_plus_plus'] += shot_percentage * point_value
    ...
    df['Morey Factor+'] = df['ast_plus_plus'] / df['total_assists']
...
def calculate_assist_plus_plus_for_year_range(start_year=1996, end_year=2017):
    df = p.DataFrame()
    for year in range(start_year, end_year):
        year_string = d.get_year_string(year)
        df = df.append(calculate_assist_plus_plus_for_year(year_string))
    return df


calculate_assist_plus_plus_for_year()